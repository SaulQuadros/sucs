...
def build_excel_template_bytes():
    ...
    for grupo, desc, params in rows:
        rec = dict(
            ...
            amostra=grupo,
            pct_retido_200=None,
            pct_pedregulho_coarse=0, pct_areia_coarse=0,
            LL=None, LP=None, Cu=None, Cc=None, organico=False, turfa=False
        )
        rec.update(params); records.append(rec)
...
with st.sidebar:
    ...
    st.download_button('Baixar planilha‑modelo (Excel)',
...
with col2:
    ...
    y_max = max(40, IP + 10, y_line_end + 5, 15)
    ax.set_xlim(0, x_max)
    ax.set_ylim(0, y_max)
    # Rótulos das guias
    ylim = ax.get_ylim()
...
with col3:
    st.subheader("Opcional (Cu/Cc e orgânicos)")
    allow_border = st.checkbox(
        "Permitir Cu/Cc também para 5–12% (1º símbolo do grupo duplo)",
        value=False,
        help="Habilita Cu/Cc quando 5–12% de finos apenas para decidir o 1º símbolo (GW/GP ou SW/SP).",
    )
    allowed_grad = coarse and ((fines < 5.0) or (allow_border and (5.0 <= fines <= 12.0)))
    use_grad = st.checkbox(
        "Usar Cu/Cc para decidir W/P",
        value=False,
        help="Cu/Cc só se aplicam a solos de granulação grossa (G/S). Em 5–12%, valem apenas para o 1º símbolo do grupo duplo.",
    ...
    if not allowed_grad:
        st.caption(f"Para habilitar Cu/Cc: grossa (retido #200 ≥ 50%) e finos < 5% "
                   f"ou, se marcado acima, também para 5–12% (apenas 1º símbolo). "
                   f"Agora: retido #200 = {pct_retido_200:.1f}%, finos = {fines:.1f}%.")
...
if st.button("Classificar (formulário acima)"):
    data = {
        ...
        "tecnico": st.session_state.get("tecnico", ""),
        "amostra": st.session_state.get("amostra", ""),
    ...
    st.text(relatorio)
    st.download_button("Baixar relatório (.txt)", relatorio, file_name=f"sucs_{st.session_state.get('amostra','amostra')}.txt")
